...
export function StaffView({ user, setActiveTab }) {
  ...
  const switchPane = (pane) => {
    setActivePane(pane);
    // Reset transient UI states when switching panes to avoid inconsistent states
    setShowAdd(false);
    setIsTeamMeetingMode(false);
    setIsFindingMeeting(false);
        ...
        <div className="space-y-3">
          ...
          <TabBar
            tabs={[
              ...
              { id: 'leadership', label: 'Leadership Lens' },
            ]}
            activeId={activePane}
            onChange={switchPane}
            wrapClassName="self-start w-fit"
          />
        </div>
        {activePane === 'team' && (
