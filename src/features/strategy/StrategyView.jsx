...
import { Map, Lightbulb, AlertTriangle, CheckCircle2, Loader2, Sparkles, PlusCircle, ClipboardList, Layers, Trash2 } from 'lucide-react';
...
export function StrategyView({ user, staff = [] }) {
  ...
  const [workspaceTab, setWorkspaceTab] = useState('sip');
    ...
    <div className="space-y-6">
      ...
      {isBudgetLens ? (
        ...
        <>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-3 flex items-center gap-2">
            <button
              className={`px-4 py-2 rounded-2xl text-sm font-semibold flex items-center gap-2 ${
                workspaceTab === 'sip' ? 'bg-indigo-600 text-white shadow' : 'text-slate-600'
              }`}
              onClick={() => setWorkspaceTab('sip')}
            >
              <ClipboardList size={16} /> School Priorities
            </button>
            <button
              className={`px-4 py-2 rounded-2xl text-sm font-semibold flex items-center gap-2 ${
                workspaceTab === 'projects' ? 'bg-indigo-600 text-white shadow' : 'text-slate-600'
              }`}
            ...
            <button
              className={`px-4 py-2 rounded-2xl text-sm font-semibold flex items-center gap-2 ${
                workspaceTab === 'ideas' ? 'bg-indigo-600 text-white shadow' : 'text-slate-600'
              }`}
              onClick={() => setWorkspaceTab('ideas')}
          >
            <Lightbulb size={16} /> Idea Lab
          </button>
        </div>

          <div className="h-[calc(100vh-100px)] overflow-y-auto space-y-6 pr-1">
